...
public class DirectByteBufferPool {
	...
	public ByteBuffer allocat(int size) {
		...
		ByteBuffer byteBuf = allocatBuffer(theChunkCount, 0, selectedPage);
		if (byteBuf == null) {
			byteBuf = allocatBuffer(theChunkCount, selectedPage, allPages.length);
		}
		return byteBuf;
	...
	public void recycle(ByteBuffer theBuf)
	{
		...
		{
			if((recycled=allPages[i].reclyeBuffer((ByteBuffer) parentBuf,startChunk,chunkCount)==true))
			{
				break;
			}
		}
	...
	private ByteBuffer allocatBuffer(int theChunkCount, int startPage, int endPage) {
